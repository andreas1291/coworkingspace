...
const render = (store, shouldHydrate) => {
  ...
  console.log('languageCountryConfig', languageCountryConfig)
  ...
  info
    .then(() => {
      ...
      if (shouldHydrate) {
        ReactDOM.hydrate(<ClientApp store={store} locale={locale} />, document.getElementById('root'));
      } else {
        ReactDOM.render(<ClientApp store={store} locale={locale} />, document.getElementById('root'));
      }
    })
    .catch(e => {
...
if (typeof window !== 'undefined') {
  ...
  if (config.dev) {
    ...
    window.app = {
      ...
      store,
      sample,
      routeConfiguration: routeConfiguration(window.location.pathname),
    };
  }
}
